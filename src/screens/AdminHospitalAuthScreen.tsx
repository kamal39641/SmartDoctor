...
export default function AdminHospitalAuthScreen({ navigation }: any) {
  ...
  const onRegister = () => {
    ...
    const res: any = registerAdmin("hospital", name, email, password, secretKey);
    if (!res.success) return Alert.alert("রেজিস্ট্রেশন ব্যর্থ", res.message);
    Alert.alert("সফল", "হাসপাতাল অ্যাডমিন রেজিস্ট্রেশন সফল");
    navigation.replace("AdminHospitalApproval");
  };

  const onLogin = () => {
    // demo behavior: if no credentials entered, allow demo login
    if (!email || !password) {
      const demoRes: any = demoLogin ? demoLogin("hospital") : null;
      ...
      // final fallback: proceed as demo
      Alert.alert("সফল", "ডেমো লগইন হয়েছে");
      navigation.replace("AdminHospitalApproval");
      return;
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>হাসপাতাল অ্যাডমিন</Text>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.inputBox}>
          ...
          <TextInput value={email} onChangeText={setEmail} placeholder="admin@hospital.com" style={styles.input} />
        </View>
        ...
        {mode === "register" && (
          <>
            <Text style={styles.label}>Hospital Admin Secret</Text>
            <View style={styles.inputBox}>
              <Ionicons name="key" size={20} color="#888" />
              <TextInput value={secretKey} onChangeText={setSecretKey} placeholder="Hospital secret" style={styles.input} secureTextEntry />
            </View>
          </>
        )}
        ...
        <View style={styles.hintBox}>
          <Text style={styles.hintText}>এই ফর্ম শুধুমাত্র হাসপাতাল-অ্যাডমিনদের জন্য। রেজিস্ট্রেশনের জন্য প্রয়োজন সঠিক Secret key।</Text>
        </View>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  header: { width: "100%", backgroundColor: "#F59E0B", paddingTop: 60, paddingBottom: 20, alignItems: "center", justifyContent: "center" },
  ...
  button: { marginTop: 18, backgroundColor: "#F59E0B", padding: 14, borderRadius: 12 },
